    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled
            ? "bg-background/95 backdrop-blur-md border-b border-bronze/20"
            : "bg-transparent"
        }`}
      >
            ...
            <div className="hidden lg:flex items-center gap-10">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  to={link.href}
                  className={`nav-link font-sans-luxury relative transition-colors ${
                    location.pathname === link.href
                      ? "text-bronze after:content-[''] after:absolute after:-bottom-1 after:left-0 after:right-0 after:h-px after:bg-bronze"
                      : "hover:text-bronze"
                  }`}
                >
                  {link.label}
